...
//			m_fadePanel [(int)mode - 1] = Mathf.Clamp (m_fadePanel [(int)mode - 1], 0.001f, 1f);
		...
		public static bool DrawSensorHandle( VertExmotionSensorBase sensor, bool AllInfo )
		{
            ...
            if (m_settingMode == eSettingsMode.SENSORS || m_settingMode == eSettingsMode.FX )
            {
                ...
                var fmh_648_71_638558862537134635 = Quaternion.identity; sensor.transform.position =
                    Handles.FreeMoveHandle(sensor.transform.position, (constUnit * .02f), Vector3.zero, CircleCap);
                ...
                if (VertExmotionBase.GetScaleFactor(sensor.transform) > 0)
                {
                    ...
                    var fmh_659_197_638558862537154713 = Quaternion.identity; radius =
                        Vector3.Distance(
                                             Handles.FreeMoveHandle(sensor.transform.position + svCam.transform.right * sensor.m_envelopRadius * VertExmotionBase.GetScaleFactor(sensor.transform), (constUnit * .02f), Vector3.zero, CubeCap)
                ...
                var fmh_678_59_638558862537164769 = Quaternion.identity; lastPos = Handles.FreeMoveHandle(lastPos, (constUnit * .01f), Vector3.zero, CircleCap);
			...
			if( m_settingMode == eSettingsMode.COLLIDERS )
			{
				...
				if( vtmCol != null)
				{
					for(int i=0; i<vtmCol.m_collisionZones.Count;++i)
					{
						...
						{
							var fmh_917_102_638558862537203786 = Quaternion.identity; float old =
								Vector3.Distance( 
								                 Handles.FreeMoveHandle(  worldColZonePos + svCam.transform.right * radius , ( constUnit*.02f ), Vector3.zero, CubeCap )
						...
						var fmh_924_132_638558862537216304 = Quaternion.identity; vtmCol.m_collisionZones[i].positionOffset = vtmCol.transform.InverseTransformPoint( Handles.FreeMoveHandle( worldColZonePos, ( constUnit*.02f ), Vector3.zero, CircleCap ) );
					}
				}
			}
